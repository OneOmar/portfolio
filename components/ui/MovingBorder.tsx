...
export function Card({
  ...
  return (
    <Component
      className={`relative md:col-span-2 overflow-hidden bg-transparent p-[1px] text-xl ${containerClassName ?? ""}`}
      style={{ borderRadius }}
      {...otherProps}
    >
      <div className="absolute inset-0" style={{ borderRadius: `calc(${borderRadius} * 0.96)` }}>
        <MovingBorder duration={duration} rx="30%" ry="30%">
          <div
            className={`h-20 w-20 bg-[radial-gradient(#0ea5e9_40%,transparent_60%)] opacity-80 ${borderClassName ?? ""}`}
          />
        </MovingBorder>
      </div>
...
export const MovingBorder = ({
  ...
  return (
    <>
      ...
      <motion.div
        style={{ position: "absolute", top: 0, left: 0, display: "inline-block", transform, willChange: "transform" }}
      >
        {children}
      </motion.div>
    </>
